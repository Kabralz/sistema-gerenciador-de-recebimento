...
// Atualiza a página a cada 10 segundos se reloadAtivo e sem AJAX pendente
setInterval(function() {
    if (reloadAtivo && !algumModalAberto() && !ajaxStatusEmAndamento) location.reload();
}, 10000);
...
// Pausa reload ao focar em qualquer select
document.addEventListener("DOMContentLoaded", function() {
...
// Se clicar fora de qualquer select, ativa reload
document.addEventListener('mousedown', function(e) {
...
// Pausa reload ao abrir o modal de conferência
function abrirModalConferencia(idAgendamento) {
    reloadAtivo = false; // Pausa reload
    fetch('get_conferencia.php?id=' + idAgendamento)
        .then(r => r.json())
...
// Atualiza a página a cada 5 segundos se reloadAtivo
setInterval(function() {
    if (reloadAtivo && !algumModalAberto()) location.reload();
}, 10000);
